...
// Fetch only "Income" type items for dropdown
$items = [];
$sqlItems = "SELECT item_code, item_name FROM items WHERE type = 'Income'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_income'])) {
    ...
    if (empty($selected_asset)) {
        ...
        if ($mysqli->query($sql)) {
            $message = "✅ New income recorded successfully!";
        } else {
...
<body>
    ...
    <div class="container">
        ...
        <form method="POST" action="">
            ...
            <button type="submit" name="save_income" class="blue-button">Save Income</button>
        </form>
    </div>
